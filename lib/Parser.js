...
class VEDirectParser extends EventEmitter {
  constructor (opts) {
    ...
    this.debug = debug('@signalk/vedirect-serial-usb')
    this.line = []
    this.data = {}
    this.cache = ''
    this.sum = 0
  ...
  _verifyCacheAndParse () {
    ...
    if (this.options.ignoreChecksum !== true && this.sum % 256 !== 0) {
  ...
  _parse () {
    ...
    if (!Object.keys(this.fields).includes(field) || typeof this.fields[field] !== 'object' || this.fields[field] === null) {
      return this.debug(`No field definition for: ${field}, ignoring`)
    }
  ...
  generateDelta () {
    ...
    if (this.data['stateOfOperation'].value != oldStateOfOperation) {
      console.log("New stateOfOperation: " + this.data['stateOfOperation'].value);
      values.push({"path": "electrical.solar.SmartSolar.chargingModeChanges", "value": this.data['stateOfOperation'].value});
      oldStateOfOperation = this.data['stateOfOperation'].value;
    }
