...
class VEDirectParser extends EventEmitter {
  constructor (opts) {
    ...
    this.debug = debug('vedirect-serial-usb')
    this.line = []
    this.data = {}
    this.cache = ''
    this.sum = 0
  ...
  _verifyCacheAndParse () {
    ...
    if (this.sum % 256 !== 0) {
      this.debug(`block checksum doesn't equal 0: ${this.sum % 256}`)
      this.cache = ''
      this.sum = 0
      return
    }
  ...
  generateDelta () {
    ...
    const values = keys.map(name => {
      ...
      this.debug('this.data[name].value: ', this.data[name].value)
      // if (this.data['solarCurrent'].value > 100) {
      //   return null
      // }
      // if (this.data['panelPower'].value > 1000) {
      //   return null
      // }
      //
      // if (this.data['voltage'].value == 0) {
      //   return null
      // }
